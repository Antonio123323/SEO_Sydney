$(document).ready(function () {
    ...
    }, "Name must be more than 2 characters, no special characters or spaces");

    $.validator.addMethod("lastusernameRegex", function (value, element) {
        return this.optional(element) || regex_first_last_name.test(value);
    }, "Last name must be more than 2 characters, no special characters or spaces");

    $.validator.addMethod("passwordRegex", function (value, element) {
        return this.optional(element) || /[a-z]/.test(value) && /[0-9]/.test(value) && /[A-Z]/.test(value) && /^[0-9A-Za-z]+$/.test(value);
    }, 'Passwords must be between 8 and 12 characters, contain letters (A-Z, a-z) and numbers (0-9). No special characters (^@()#*+/"?!=.{}~&) and spaces');

    $.validator.addMethod("phoneRegex", function (value, element) {
        return this.optional(element) || /^(\d[- ]?){7,11}$/.test(value);
    }, "The phone must have between 7 and 11 characters, no special characters");
    ...
    $(function () {
        ...
        form.validate({
            ...
            messages: {
                first_name: {
                    required: "The field for name is required",
                    minlength: "The name must have de least 2 characters",
                    maxlength: "The name can be up to 60 characters long",
                },

                last_name: {
                    required: "The last name field is required",
                    minlength: "The surname must have de least 2 characters",
                    maxlength: "Surnames can be up to 60 characters long",
                },
                password: {
                    required: "The password field is mandatory",
                    minlength: "The password must be de least 8 characters long",
                    maxlength: "Passwords can be up to 12 characters long",
                },
                email: {
                    required: "The field for email is required",
                ...
                .done(function (data) {
                    ...
                    .fail(function (jqXHR, textStatus, errorThrown) {
                        ...
                        if (jqXHR.status == 400) {
                            ...
                            alert('Error in the registration form');
                            console.log(jqXHR)
                        }
                    });
